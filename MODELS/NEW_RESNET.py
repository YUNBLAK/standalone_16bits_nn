...
class ResNetModel(tf.keras.Model):
    def __init__(self,
        ...
        if num_layers not in (20, 32, 44, 56, 110, 156):
        ...
        #self._kernel_regularizer = regularizers.l1(weight_decay)
        if l2_switch:
            self._kernel_regularizer = regularizers.l2(weight_decay)
        else:
            self._kernel_regularizer = regularizers.l1(weight_decay)
        
        self._init_conv = layers.Conv2D(16, 3, 1, 'same', use_bias=True, kernel_regularizer=self._kernel_regularizer)
